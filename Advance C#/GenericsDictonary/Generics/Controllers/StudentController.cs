...
namespace Generics.Controllers
{
    ...
    {
        private readonly IGenericService<Stu01> _studentService = new BLStudentService();
        ...
        public IHttpActionResult GetStudents()
        {
            try
            {
                Dictionary<int, Stu01> students = _studentService.GetAll();
                return Ok(students);
            }
            catch (Exception ex)
            {
        ...
        public IHttpActionResult AddStudent([FromBody] Stu01 objStudent)
        {
            try
            {
                Dictionary<int, Stu01> updatedStudents = _studentService.Insert(objStudent);
        ...
        public IHttpActionResult DeleteStudent(int id)
        {
            try
            {
                Dictionary<int , Stu01> updatedStudents = _studentService.Delete(id);
                return Ok(updatedStudents);
            }
            catch (Exception ex)
            {
